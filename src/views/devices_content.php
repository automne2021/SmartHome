        ...
        <div class="device-types">
            ...
            <?php foreach (array_keys($devicesByType) as $type): ?>
                <div class="device-type-tab" data-type="<?php echo $type; ?>">
                    ...
                    $icon = 'question';
                    switch ($type) {
                        case 'lamp': $icon = 'lightbulb'; break;
                        case 'fan': $icon = 'fan'; break;
                        case 'door': $icon = 'door-open'; break;
                    }
                    ?>
...
function renderDeviceCards($devices) {
    ...
    foreach ($devices as $device) {
        ...
        $output .= '<div class="device-card ' . ($device['status'] == 'on' ? 'device-on' : 'device-off') . '" data-device-id="' . $device['id'] . '">';
        
        // Status badge
        $output .= '<div class="device-status-badge"><span>' . ucfirst($device['status']) . '</span></div>';
        ...
        $output .= '<span class="status-dot status-' . ($device['status'] == 'on' ? 'on' : 'off') . '"></span>';
        $output .= '<span id="status-' . $device['id'] . '" class="status-text">' . ucfirst($device['status']) . '</span>';
        $output .= '</div>';
        $output .= '</div>';
        ...
        $output .= '<button class="btn ' . ($device['status'] == 'on' ? 'btn-off' : 'btn-on') . ' device-button" ';
        $output .= 'data-device-id="' . $device['id'] . '" ';
        $output .= 'data-action="' . ($device['status'] == 'on' ? 'turn_off' : 'turn_on') . '">';
        $output .= '<i class="fas fa-' . ($device['status'] == 'on' ? 'power-off' : 'play') . '"></i> ';
        $output .= ($device['status'] == 'on' ? 'Turn Off' : 'Turn On');
        $output .= '</button>';
        $output .= '</div>';
